...
{
	auto query = L"CREATE PROCEDURE [dbo].[%s] %s AS BEGIN %s END";
...
{
	auto query = L"ALTER PROCEDURE [dbo].[%s] %s AS	BEGIN %s END";
...
wstring Helpers::DataType2String(DataType type)
{
	switch (type)
	{
	case DataType::DateTime2: return L"DateTime2";
...
{
	std::wregex reg(L"([a-z|0-9]+)(\\((max|\\d+)\\))?");
	std::wcmatch ret;
	...
	if (_wcsicmp(ret[1].str().c_str(), L"DateTime2") == 0) return DataType::DateTime2;
