...
namespace SP
{
	...
	public:
		...
		template <int32 N>
		void Out_TableName(OUT WCHAR (&value)[N]) { BindCol(1, value); }

		template <int32 N>
		void Out_ColumnName(OUT WCHAR (&value)[N]) { BindCol(2, value); }
		...
		template <int32 N>
		void Out_DefaultDefinition(OUT WCHAR (&value)[N]) { BindCol(11, value); }

		template <int32 N>
		void Out_DefaultConstraintName(OUT WCHAR (&value)[N]) { BindCol(12, value); }
	};
	...
	public:
		...
		template <int32 N>
		void Out_IndexName(OUT WCHAR (&value)[N]) { BindCol(1, value); }
		...
		template <int32 N>
		void Out_ColumnName(OUT WCHAR (&value)[N]) { BindCol(7, value); }
	};
	...
	public:
		...
		template <int32 N>
		void Out_Name(OUT WCHAR (&value)[N]) { BindCol(0, value); }
...
bool DBSynchronizer::Synchronize(const WCHAR* path)
{
	...
	GConsoleLogger->WriteStdOutWithTime(Color::BLUE, L"[DB] XML 동기화 완료\n");
...
void DBSynchronizer::ParseXmlDB(const WCHAR* path)
{
	...
	for (XmlNode& table : tables)
	{
		...
		{
			auto i = make_shared<DBModel::Index>();
	...
	{
		auto p = make_shared<DBModel::Procedure>();
...
bool DBSynchronizer::GatherDBTables()
{
	...
	while (getDBTables.Fetch())
	{
		...
		{
			return table->_objectId == objectId;
		});
		...
		auto column = make_shared<DBModel::Column>();
		{
...
bool DBSynchronizer::GatherDBIndexes()
{
	...
	while (getDBIndexes.Fetch())
	{
		...
		auto findIndex = std::find_if(indexes.begin(), indexes.end(), [indexId](DBModel::IndexRef& index)
		{
			return index->_indexId == indexId;
		});
		if (findIndex == indexes.end())
		{
			auto index = make_shared<DBModel::Index>();
			{
		...
		auto findColumn = std::find_if(columns.begin(), columns.end(), [columnId](DBModel::ColumnRef& column)
		{
			return column->_columnId == columnId;
		});
...
bool DBSynchronizer::GatherDBStoredProcedures()
{
	...
	{
		auto proc = make_shared<DBModel::Procedure>();
		{
			proc->_name = name;
...
void DBSynchronizer::CompareDBModel()
{
	...
	for (DBModel::TableRef& dbTable : _dbTables)
	{
		...
		else
		{
			if (_xmlRemovedTables.find(dbTable->_name) != _xmlRemovedTables.end())
			{
				...
				_updateQueries[DropTable].push_back(
					DBModel::Helpers::Format(L"DROP TABLE [dbo].[%s]", dbTable->_name.c_str()));
			}
		}
	}
	...
	for (auto& mapIt : xmlTableMap)
	{
		...
		_updateQueries[CreateTable].push_back(
			DBModel::Helpers::Format(L"CREATE TABLE [dbo].[%s] (%s)", xmlTable->_name.c_str(), columnsStr.c_str()));
		...
		for (DBModel::ColumnRef& xmlColumn : xmlTable->_columns)
		{
			...
			_updateQueries[DefaultConstraint].push_back(DBModel::Helpers::Format(
		...
		for (DBModel::IndexRef& xmlIndex : xmlTable->_indexes)
		{
			GConsoleLogger->WriteStdOut(Color::YELLOW, L"Creating Index : [%s] %s %s [%s]\n", xmlTable->_name.c_str(),
			                            xmlIndex->GetKeyText().c_str(), xmlIndex->GetTypeText().c_str(),
			                            xmlIndex->GetUniqueName().c_str());
			if (xmlIndex->_primaryKey || xmlIndex->_uniqueConstraint)
			{
				_updateQueries[CreateIndex].push_back(DBModel::Helpers::Format(
...
{
	for (int32 step = 0; step < Max; step++)
	{
...
void DBSynchronizer::CompareTables(DBModel::TableRef dbTable, DBModel::TableRef xmlTable)
{
	...
	for (DBModel::ColumnRef& dbColumn : dbTable->_columns)
	{
		...
		else
		{
			GConsoleLogger->WriteStdOut(Color::YELLOW, L"Dropping Column : [%s].[%s]\n", dbTable->_name.c_str(),
			                            dbColumn->_name.c_str());
			if (dbColumn->_defaultConstraintName.empty() == false)
				_updateQueries[DropColumn].push_back(DBModel::Helpers::Format(
					L"ALTER TABLE [dbo].[%s] DROP CONSTRAINT [%s]", dbTable->_name.c_str(),
					dbColumn->_defaultConstraintName.c_str()));

			_updateQueries[DropColumn].push_back(DBModel::Helpers::Format(
				L"ALTER TABLE [dbo].[%s] DROP COLUMN [%s]", dbTable->_name.c_str(), dbColumn->_name.c_str()));
		}
	}
	...
	for (auto& mapIt : xmlColumnMap)
	{
		...
		GConsoleLogger->WriteStdOut(Color::YELLOW, L"Adding Column : [%s].[%s]\n", dbTable->_name.c_str(),
		                            xmlColumn->_name.c_str());
		_updateQueries[AddColumn].push_back(DBModel::Helpers::Format(L"ALTER TABLE [dbo].[%s] ADD %s %s",
		                                                             dbTable->_name.c_str(), xmlColumn->_name.c_str(),
		                                                             xmlColumn->_typeText.c_str()));

		if (xmlColumn->_nullable == false && xmlColumn->_default.empty() == false)
		{
			_updateQueries[AddColumn].push_back(DBModel::Helpers::Format(
				L"SET NOCOUNT ON; UPDATE [dbo].[%s] SET [%s] = %s WHERE [%s] IS NULL",
				dbTable->_name.c_str(), xmlColumn->_name.c_str(), xmlColumn->_default.c_str(),
				xmlColumn->_name.c_str()));
		}

		if (xmlColumn->_nullable == false)
		{
			_updateQueries[AddColumn].push_back(DBModel::Helpers::Format(L"ALTER TABLE [dbo].[%s] ALTER COLUMN %s",
			                                                             dbTable->_name.c_str(),
		...
		if (xmlColumn->_default.empty() == false)
		{
			_updateQueries[AddColumn].push_back(DBModel::Helpers::Format(
				...
				dbTable->_name.c_str(), dbTable->_name.c_str(), xmlColumn->_name.c_str(), xmlColumn->_default.c_str(),
				xmlColumn->_name.c_str()));
		}
	}
	...
	for (DBModel::IndexRef& dbIndex : dbTable->_indexes)
	{
		...
		if (findIndex != xmlIndexMap.end() && _dependentIndexes.find(dbIndex->GetUniqueName()) == _dependentIndexes.
			end())
		{
		...
		else
		{
			GConsoleLogger->WriteStdOut(Color::YELLOW, L"Dropping Index : [%s] [%s] %s %s\n", dbTable->_name.c_str(),
			                            dbIndex->_name.c_str(), dbIndex->GetKeyText().c_str(),
			                            dbIndex->GetTypeText().c_str());
			if (dbIndex->_primaryKey || dbIndex->_uniqueConstraint)
				_updateQueries[DropIndex].push_back(DBModel::Helpers::Format(
					L"ALTER TABLE [dbo].[%s] DROP CONSTRAINT [%s]", dbTable->_name.c_str(), dbIndex->_name.c_str()));
			else
				_updateQueries[DropIndex].push_back(
					DBModel::Helpers::Format(L"DROP INDEX [%s] ON [dbo].[%s]", dbIndex->_name.c_str(),
					                         dbTable->_name.c_str()));
		}
	}
	...
	for (auto& mapIt : xmlIndexMap)
	{
		...
		if (xmlIndex->_primaryKey || xmlIndex->_uniqueConstraint)
		{
			_updateQueries[CreateIndex].push_back(DBModel::Helpers::Format(
				L"ALTER TABLE [dbo].[%s] ADD CONSTRAINT [%s] %s %s (%s)",
				dbTable->_name.c_str(), xmlIndex->CreateName(dbTable->_name).c_str(), xmlIndex->GetKeyText().c_str(),
				xmlIndex->GetTypeText().c_str(), xmlIndex->CreateColumnsText().c_str()));
		}
		else
		{
			_updateQueries[CreateIndex].push_back(DBModel::Helpers::Format(L"CREATE %s INDEX [%s] ON [dbo].[%s] (%s)",
			                                                               xmlIndex->GetTypeText(),
...
void DBSynchronizer::CompareColumns(DBModel::TableRef dbTable, DBModel::ColumnRef dbColumn,
                                    DBModel::ColumnRef xmlColumn)
{
	uint8 flag = 0;
	...
	if (dbColumn->_nullable != xmlColumn->_nullable)
		flag |= Nullable;
	if (dbColumn->_identity != xmlColumn->_identity || (dbColumn->_identity && dbColumn->_incrementValue != xmlColumn->
		_incrementValue))
		flag |= Identity;
	...
	if (flag)
	{
		GConsoleLogger->WriteStdOut(Color::YELLOW, L"Updating Column [%s] : (%s) -> (%s)\n", dbTable->_name.c_str(),
		                            dbColumn->CreateText().c_str(), xmlColumn->CreateText().c_str());
	}

	// 연관된 인덱스가 있으면 나중에 삭제하기 위해 기록한다.
	if (flag & (Type | Length | Nullable))
	{
		for (DBModel::IndexRef& dbIndex : dbTable->_indexes)
	...
	if (flag & Default)
	{
		if (dbColumn->_defaultConstraintName.empty() == false)
		{
			_updateQueries[AlterColumn].push_back(DBModel::Helpers::Format(
...
void DBSynchronizer::CompareStoredProcedures()
{
	...
	for (DBModel::ProcedureRef& dbProcedure : _dbProcedures)
	{
		...
		if (findProcedure != xmlProceduresMap.end())
		{
			...
			if (DBModel::Helpers::RemoveWhiteSpace(dbProcedure->_fullBody) !=
				DBModel::Helpers::RemoveWhiteSpace(xmlBody))
			{
				GConsoleLogger->WriteStdOut(Color::YELLOW, L"Updating Procedure : %s\n", dbProcedure->_name.c_str());
				_updateQueries[StoredProcecure].push_back(xmlProcedure->GenerateAlterQuery());
			}
	...
	{
		GConsoleLogger->WriteStdOut(Color::YELLOW, L"Updating Procedure : %s\n", mapIt.first.c_str());
		_updateQueries[StoredProcecure].push_back(mapIt.second->GenerateCreateQuery());
	}
}
