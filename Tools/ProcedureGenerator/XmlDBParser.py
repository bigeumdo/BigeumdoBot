...
def ParseColumns(node, tables):
    ...
    exists_idx = max(query.rfind('EXISTS'), query.rfind('exists'))
    if exists_idx > 0:
        columns = ParseWords(query, tables, 0, exists_idx)
    else:
        columns = ParseWords(query, tables, 0, -1)

    return columns

def ParseWords(query, tables, s, e):
    columns = []
    select_idx = max(query.rfind('SELECT', s, e), query.rfind('select', s, e))
    from_idx = max(query.rfind('FROM', s, e), query.rfind('from', s, e))
